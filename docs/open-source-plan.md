...
2026-09-08 当日选型结论仍有效。之后原型已做到：Word 适配层隔离写入、老师窗口与内置 pi、大连财经 2026 格式抽查、历史字符串匹配收紧（V0.2）、历史问题规则化结构（V0.3）、有密钥时由 Pi 判定历史复发并做主张-证据取证、证据不足不批（V0.4），以及真实模型金标入口与工具轨迹评分（V0.5）。召回仍是字符串，不是语义历史召回。默认测试仍是 faux。仍未完成：全文排版验收、其余论证结构类型、语义历史召回、live eval 通过后的质量结论、正式部署与预算控制。授权真稿不进本仓库。
