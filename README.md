...
**这是可本地试用的原型（当前 V0.2），不是已经验收过的学校正式产品。**老师在窗口里选文件、确认历史问题、点审查；结果仍是带批注和修订的 Word 副本。Windows 目录包内置 Node 与 pi，老师不用再安装 Python 或 Node。

学校格式条款已按大连财经学院 2026 届手册做成确定性检查。授权真稿只在本机对照，不进本仓库。历史复查目前是收紧后的字符串匹配，不是语义判断。
...
| V0.3 | 下一步 | 把历史问题结构化，命中后由模型确认是不是同一条问题 |
| V0.4 | 之后 | 论证结构（B）只做重大缺口提示，例如主张缺少证据 |

已知限制：中英文混排时正文字数检查会偏短；标题原文没改时，标题下的内容问题仍可能被当成复发。
...
本机已用 Microsoft Word 打开过审改副本并读到助手批注；学校条款抽查与授权真稿对照也做过。仍未完成：全文排版验收、语义级历史识别、带密钥的 pi 端到端审改质量评估。授权真稿和学校文件全文不进本仓库。
