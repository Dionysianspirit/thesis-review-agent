...
**[下载 论文审改助手 Windows 版（最新 Release）](https://github.com/Dionysianspirit/thesis-review-agent/releases/latest)**

- Windows 10/11，已安装 Microsoft Word；**不需要**安装 Python 或 Node，运行时已全部内置。
- 解压后双击 `论文审改助手.exe`。首次启动如被 SmartScreen 拦截，选择「仍要运行」。
- 密钥只保存在本机 `%APPDATA%\ThesisReviewAgent\`，不打进安装包；不配密钥也能离线初筛（格式、语言规则 + 历史字符串复查）。
- 可用界面里的「载入演示稿」先走一遍模拟学生小周的完整流程。

**这是可本地试用的原型（当前 V0.4），不是已经验收过的学校正式产品。**老师在窗口里选文件、确认历史问题、点审查；结果仍是带批注和修订的 Word 副本。Windows 目录包内置 Node 与 pi，老师不用再安装 Python 或 Node。
...
已知限制：中英文混排时正文字数检查会偏短；无密钥时，标题原文没改仍可能把标题下的内容问题当成复发；过短的老师批注仍不会进入字符串召回；主张-证据取证在未配置密钥时不会运行。有密钥时当前自动化测试走的是预定工具路径，证明链路能跑，不证明真实模型对真稿会走出正确取证路径。Pi 调用失败时格式规则仍离线可用，历史复发改为不写，避免把未确认的字符串命中当成再次犯错。
...
本机已用 Microsoft Word 打开过审改副本并读到助手批注；学校条款抽查与授权真稿对照也做过。仍未完成：全文排版验收、带密钥的 pi 端到端审改质量评估。授权真稿和学校文件全文不进本仓库。
